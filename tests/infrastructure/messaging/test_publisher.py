...
class TestPublishEvent:
    """Test suite for publish_event function."""

    @pytest.mark.asyncio
    async def test_publish_event_basic(self):
        """Test publishing an event to a queue."""
        ...
        with patch(
            ...
            # Verify connection and channel were created
    ...
    async def test_publish_event_with_headers(self):
        """Test publishing an event with custom headers."""
        mock_connection = AsyncMock(spec=aio_pika.RobustConnection)
        mock_channel = AsyncMock(spec=aio_pika.Channel)
        mock_queue = AsyncMock()
        mock_exchange = AsyncMock()
    ...
    async def test_publish_event_routing_key(self):
        """Test that message is published with correct routing key."""
        mock_connection = AsyncMock(spec=aio_pika.RobustConnection)
        mock_channel = AsyncMock(spec=aio_pika.Channel)
        mock_queue = AsyncMock()
        mock_exchange = AsyncMock()
    ...
    async def test_publish_event_complex_data(self):
        """Test publishing complex nested event data."""
        mock_connection = AsyncMock(spec=aio_pika.RobustConnection)
        mock_channel = AsyncMock(spec=aio_pika.Channel)
        mock_queue = AsyncMock()
        mock_exchange = AsyncMock()
